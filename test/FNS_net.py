...
class GridPositions(Positions):
    ...
    def __call__(self, shape):
        ...
        for s, n in zip(self.size, shape):
            grids.append(np.linspace(0, s, n, endpoint=False))
...
class DistanceDependent(TwoEndConnector):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if domain_size is None:
            combined_positions = jnp.vstack([self.positions_pre, self.positions_post])
            self.domain_size = jnp.max(combined_positions, axis=0) - jnp.min(
                combined_positions, axis=0
            )
        else:
            self.domain_size = jnp.asarray(domain_size)
    ...
    def build_mat(self):
        ...
        def compute_distance_with_boundary(
            ...
            delta_periodic = (
                jnp.mod(delta + domain_size / 2, domain_size) - domain_size / 2
            )
            ...
            adjusted_pos_pre_reflecting = jnp.clip(pos_pre, 0, domain_size)
            adjusted_pos_post_reflecting = jnp.clip(pos_post, 0, domain_size)
            dist_reflecting = distance_metric(
                adjusted_pos_pre_reflecting, adjusted_pos_post_reflecting
            )

            # Absorbing boundary adjustment
            outside_pre = (pos_pre < 0) | (pos_pre > domain_size)
            outside_post = (pos_post < 0) | (pos_post > domain_size)
...
class FNSCircuit(bp.DynSysGroup):
    def __init__(self, num_exc, num_inh, method="exp_auto"):
        ...
        conn_E2I = bp.connect.FixedProb(prob=1.0)
        conn_I2E = bp.connect.FixedProb(prob=1.0)
        conn_I2I = bp.connect.GridEight()

        # Synapses
        self.E2E = AMPASyn(self.E, self.E, delay=10.0, conn=conn_E2E)
        self.E2I = AMPASyn(self.E, self.I, delay=0.0, conn=conn_E2I)
        self.I2E = AMPASyn(self.I, self.E, delay=0.0, conn=conn_I2E)
        self.I2I = AMPASyn(self.I, self.I, delay=0.0, conn=conn_I2I)
...
if __name__ == "__main__":
    ...
    if False:
        nx.draw(G, pos=positions, connectionstyle="arc3,rad=0.5")
        plt.show()

    inputs = np.zeros(FNSnet.E.size)  # Want to get one neuron
    inputs[0:5] = 500.0
    inputs[-6:-1] = 500.0
    print(inputs)
    print(FNSnet.E.size)
    runner = bp.DSRunner(
        ...
        inputs=[("Ein.input", inputs.flatten()), ("Iin.input", 0.0)],
    )
    runner.run(500.0)
